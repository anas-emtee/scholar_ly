...
<?php
	...
	if(isset($_POST["checkout"]) && isset($_POST["tranxid"])){
		...
		$user = $_POST["user"];
		$user_lvl = $_SESSION["logged_user"]["level"];
		$today = date("Y-m-d");
		$end = date("Y-m-d", strtotime( date( "Y-m-d", strtotime( date("Y-m-d") ) ) . "+".$unit." month" ) );

		$con = Dbcon();
		$tranx_qr = "INSERT INTO `transaction_record` (`user`, `pay_amount`, `tranxid`, `discount`, `discount_code`, `items`) VALUES ('$user', '$amount', '$tranxid', '$discount', '$discode', '$packid' )";
		if(mysqli_query($con, $tranx_qr)or die(mysqli_error($con))){
					$item_qr = "INSERT INTO `subscriptions` (`scid`, `sctype`, `sublevel`, `tranxid`, `user`, `start`, `end`) VALUES ('$sub_item', 'class', '$user_lvl', '$tranxid', '$user', '$today', '$end')";
					mysqli_query($con, $item_qr)or die(mysqli_error($con));
		}
		mysqli_close($con);
	}
	$RETURN_URL = "subscription_confirm.php?items=".$items."&tranxid=".$tranxid."&userid=".$user."&discode=".$discode;
?>
